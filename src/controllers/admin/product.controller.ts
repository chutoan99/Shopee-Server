...
const ProductController = {
  GetAllProduct: async (req: any, res: Response) => {
    try {
      ...
      ProductService.GetAllProduct({ shopid, ...query }).then(
        (response: any) => {
  ...
  AddProduct: async (req: any, res: Response) => {
    try {
      ...
      ProductService.AddProduct(shopid, payload, fileData).then(
        (response: any) => {
          res.status(200).json(response);
        }
      );
  ...
  UpdateProduct: async (req: Request, res: Response) => {
    try {
      ...
      const fileData = req.file;
      const payload = req.body;
      ProductService.UpdateProduct(itemid, fileData, payload).then(
        (response: any) => {
          res.status(200).json(response);
        }
      );
